...
public class Main {
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for (int i = 0; i < stringList.size(); i++) {
            buff = stringList.get(i);
            for (int j = 0; j < buff.length(); j++) {
                if (buff.charAt(j) > 64 && buff.charAt(j) < 91) {
                   valid = false;
                }
            }
             if (valid) newList.add(stringList.get(i));
             valid = true;
        }
        return newList;
    }
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
       ...
       for(Map.Entry <String, Integer> entry : shopCart.entrySet()) {
           if (entry.getValue() <= 300) newMap.put(entry.getKey(), entry.getValue());
       }
